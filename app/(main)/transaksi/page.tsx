...
export default async function TransactionsPage() {
  ...
  const { data: transactions } = membership ? await supabase.from("transactions").select("id, amount, transaction_type, transaction_date, note, categories(name), accounts(name), household_members(display_name)").eq("household_id", membership.household_id).order("transaction_date", { ascending: false }).order("created_at", { ascending: false }) : { data: [] };
  return <><div className="page-heading"><div><p className="eyebrow">Catatan keuangan</p><h1>Transaksi</h1><p className="muted">Semua pemasukan dan pengeluaran household.</p></div><Link href="/transaksi/baru" className="primary-button">+ Catat transaksi</Link></div><TransactionList transactions={(transactions || []) as unknown as React.ComponentProps<typeof TransactionList>["transactions"]} /></>;
}