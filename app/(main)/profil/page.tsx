...
export default async function ProfilePage() {
  ...
  const { data: membership } = await supabase.from("household_members").select("id, household_id, display_name, households(name, invite_code)").eq("auth_user_id", user!.id).eq("is_active", true).maybeSingle();
