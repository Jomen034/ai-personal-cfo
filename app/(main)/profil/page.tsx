...
export default async function ProfilePage() {
  ...
  return <><div className="page-heading page-header-safe"><div><p className="eyebrow">Ruangmu</p><h1>Profil</h1><p className="muted">{membership?.display_name || user?.email}</p></div></div><section className="profile-grid"><article className="info-panel"><span className="eyebrow">Kode undangan</span><strong className="invite-code">{household?.invite_code || "Belum tersedia"}</strong><p className="muted">Bagikan kode ini kepada satu pengelola keuangan lainnya.</p></article><article className="info-panel"><span className="eyebrow">Household</span><h2>{household?.name || "Belum diatur"}</h2><p className="muted">{membership?.display_name}</p></article></section><section className="content-section"><div className="section-heading"><div><p className="eyebrow">Akun</p><h2>Sumber uang</h2></div><Link href="/akun" className="ghost-link">Kelola akun →</Link></div><p className="muted">Kelola rekening dan saldo di halaman Akun.</p></section><section className="content-section"><div className="section-heading"><p className="eyebrow">Lainnya</p><h2>Keamanan &amp; Privasi</h2></div><p className="muted">Pelajari bagaimana data dan privasi kamu dilindungi.</p><Link href="/keamanan-privasi" className="outline-button mt-12">Buka halaman Keamanan &amp; Privasi →</Link><form action="/api/auth/signout" method="post" className="mt-12"><button className="outline-button logout-button" type="submit">Keluar</button></form></section></>;
}
